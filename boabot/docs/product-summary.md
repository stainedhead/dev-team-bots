...
- Optionally joins a Slack workspace (Socket Mode), responding to DMs and @mentions, and/or a [Buzz](https://github.com/block/buzz) (Nostr-native) workspace as a first-class, cryptographically identified agent, responding to channel/group @mentions, direct 1:1 NIP-17 encrypted DMs, and in-thread replies without re-@mentioning — all through the same dispatch, budget, and autonomy controls.
- Every `boabot-team` persona with its own `buzz:` config block gets its own Buzz identity and its own relay connection, all running as goroutines in one shared `boabot` process alongside one orchestrator web UI — true multi-agent Buzz conversations, not a single process-wide identity. A Buzz-dispatched task (whether triggered by a channel @mention, a direct message, or an in-thread reply) creates a real, `bot_name`-tagged Task and Kanban board item, visible live in the orchestrator UI as it progresses, exactly like a task dispatched from the web UI's chat interface. DM reachability activates automatically whenever a persona's channel Buzz identity does — there is no separate DM key or config flag, and an unauthorized DM sender is silently ignored by default (see `user-docs/Buzz-Adoption-Config.md` for the security implications of leaving the author gate unconfigured). See `user-docs/Buzz-Adoption-Config.md` and `user-docs/buzz-multi-agent-getting-started.md`.
- Alternatively, a single persona can be registered as a `buzz-acp` custom harness (`boabot -acp`), speaking the [Agent Client Protocol](https://agentclientprotocol.com/) over stdio — a lighter-weight distribution path than the native Buzz integration, for the case where a persona shouldn't own its own relay identity/key. It shares the same task-execution engine as native mode, including `models.chat_provider` support, conversation continuity across turns, recurring-task scheduling, and per-task board visibility, and — when the persona's config enables them — board-completion, plugin, and CLI tools; the one capability it cannot offer is a mid-task clarifying question, which is blocked on an unstable upstream ACP protocol extension `buzz-acp` doesn't implement (see `docs/architectural-decision-record.md` ADR-B030). See `user-docs/ACP-Harness-Adoption-Config.md`.
