...
- **The persona can now actually answer questions about board and task state.** Alongside `complete_board_item`, a persona with a board store activated also gets `list_board_items` (real board items, not a guess) and, when its task store is activated, `list_my_tasks` (its own scheduled/running tasks). Ask "what's on the board" or "what's scheduled" over Buzz and the reply reflects real state — this closes a gap that previously affected every channel equally, including native mode's own web-UI chat, not something specific to ACP mode or Buzz.
