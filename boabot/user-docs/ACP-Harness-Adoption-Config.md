...
- **No filesystem/tool differences from native mode.** The MCP client, memory store, and vector store are constructed the same way; the same tools are available — including a configured `models.chat_provider`, the persona's board-completion tool (unless `bot.type` is `tech-lead`), plugin tools (when `orchestrator.plugins.install_dir` is set), and CLI-agent tools (per `orchestrator.cli_tools.<tool>.enabled`) — all activated under the exact same `config.yaml` settings native mode uses, no separate ACP-specific configuration needed.
