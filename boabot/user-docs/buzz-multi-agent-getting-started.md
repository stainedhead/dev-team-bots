...
- **Buzz has no DM support today.** Only channel/group `@mention`s are handled by any persona; direct-message listening is not implemented (see `docs/architectural-decision-record.md` and the feature spec's Non-Goals for why).
