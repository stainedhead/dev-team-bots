...
- **Phase C — Secret storage callers.** `main.go` migration (FR-046), `SlackConfig` secret path (FR-047/048), diagnostic command (FR-050), `boabotctl` subcommands (FR-049).
...
Per the PRD's acceptance criteria (51 total, `boabot-buzz-support-PRD.md`). This implementation run's definition of done: all P0-scope functional requirements implemented and unit-tested, `go build`/`go vet`/`golangci-lint run`/`go test -race ./...` passing, domain/application coverage ≥90% and not regressed, and every acceptance criterion either passing (unit-testable) or explicitly flagged as needing manual verification (infrastructure-dependent).
