...
- [ ] A Buzz request can update or cancel a previously-created task (immediate or scheduled), reflected in the Tasks UI. Left unchecked: shipped scope is narrower than as originally written — see FR-008 below and implementation-notes.md item 8. There is no in-place task mutation in either the chat or Buzz path, and cancellation only ever applies to a not-yet-confirmed pending intent (an already-dispatched/running task cannot be cancelled or updated from Buzz). `TestBuzzTaskBridge_Dispatch_Cancellation_NoTaskNoBoardItem` covers the narrower pending-intent-cancel capability that does exist.
