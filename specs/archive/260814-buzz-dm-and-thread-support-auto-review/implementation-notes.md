...
**Accepted content-normalization gap (found during self-review, not the original finding):** `Monitor.HandleResult` normalizes the relay-published text -- `output := p.Output; if !p.Success && p.Error != "" { output = p.Error }; if output == "" { output = "(no output)" }` -- before publishing. Pre-FR-301, `recordOutbound` recorded that *normalized* text as the correctly-threaded ChatStore copy. Post-FR-301, `handleSharedTaskResult` (the only remaining writer) records raw `p.Output` verbatim, which never sees `p.Error`/the `"(no output)"` fallback. So a failed task with an empty `Output` and a non-empty `Error` now has an empty ChatStore/FR-206-replay record, even though the human-facing relay reply still shows the error text. This is accepted as-is, not fixed, because `handleSharedTaskResult` is the shared, source-agnostic handler spec.md's own risk table already flags ("must not affect non-Buzz task result recording") -- Chat/Board/Operator-sourced tasks already record raw `p.Output` today, so normalizing content here would be a real behaviour change for every source, not a Buzz-scoped fix, and is out of FR-301's scope (a deduplication/threading fix, not a content-normalization one). Pinned by `TestHandleSharedTaskResult_BuzzTask_ErrorOnly_RecordsRawOutputVerbatim` so this is a recorded decision, not a silent regression.
